...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/25 15:06:20 by draynor           #+#    #+#             */
/*   Updated: 2018/12/25 16:14:22 by draynor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

long long int				ft_convert_o(t_block_prntf *b)
{
...
void						ft_print_o(t_block_prntf *b)
{
	char					*str;
	int						len;

	if (b->sz.j)
		str = ft_ultoa_base(ft_convert_o(b), 8);
	else
		str = ft_lltoa_base(ft_convert_o(b), 8);
	len = (int)ft_strlen(str);
	if (b->fl.m)
		ft_print_o_2(b, str, len);
	else
	{
		if (str[0] == '-')
		{
		...
		ft_print_o_3(b, str, len);
	}
	free(str);
}

void						ft_work_o_2(t_block_prntf *b,
...
void						ft_work_o_0(t_block_prntf *b, const char *format)
{
	int						i;
